...
public interface CommentRepository extends JpaRepository<Comment, Long>, JpaSpecificationExecutor<Comment> {
    ...
    @Query(
        value = "select distinct comment from Comment comment left join fetch comment.user left join fetch comment.recipe",
    ...
    @Query("select distinct comment from Comment comment left join fetch comment.user left join fetch comment.recipe")
    List<Comment> findAllWithToOneRelationships();

    @Query("select comment from Comment comment left join fetch comment.user left join fetch comment.recipe where comment.id =:id")
